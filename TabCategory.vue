<template>
  <div>
    ...
    <vxe-table
      id="advertising-proportion-analysis-new-category"
      ref="table"
    ...
    >
      <vxe-table-column field="categoryName" title="类目" min-width="150" fixed="left">
        <template #default="{ row }">
          <span style="cursor: pointer;color: #409eff" @click="handleTableClick(row)">{{ row.categoryName }}</span>
        </template>
      </vxe-table-column>
...
export default {
  components: {
    ErpSelection, CascadeSelector, DialogDetail,
  },
  props: {
    lastTime: {
      type: String,
  ...
  methods: {
    handleTableClick(row) {
      const params = {
        ...this.saveForm,
        categoryIdList: [row.categoryId],
      }
      this.$emit('handleTableClick', params)
    },
    handlShowDetail(row) {
      this.detailRow = row
    ...
    checkColumnMethod({ column }) {
      if (column.property === 'categoryName') return false
      return true
    },
