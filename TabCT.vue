<template>
  <div>
    ...
    <vxe-table
      id="advertising-proportion-analysis-new-ct"
      ref="table"
    ...
    >
      <vxe-table-column field="combatTeam" title="CT" min-width="120" fixed="left">
        <template #default="{ row }">
          <span style="cursor: pointer;color: #409eff" @click="handleTableClick(row)">{{ row.combatTeam }}</span>
        </template>
      </vxe-table-column>
      ...
      <vxe-table-column field="turnoverDays" title="周转天数" min-width="120" sortable :formatter="formatter">
        <template #header>
          ...
          <el-tooltip
            effect="dark"
            content="FBA可售/平均销量，平均销量等于，条件范围内销量/销量大于0的天数"
            placement="top"
          >
            <i class="el-icon-question" />
          </el-tooltip>
...
export default {
  ...
  methods: {
    handleTableClick(row) {
      const params = {
        ...this.saveForm,
        combatTeamList: [row.combatTeamKey],
      }
